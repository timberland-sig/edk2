...
/**
  ...
  @param[in, out]  NvData         The target config data structure.
  ...
  )
{
  ...
  UINTN                      DnsServerListCount = 0;
  UINTN                      DataSize           = 0;
  CHAR16                     *HostName          = NULL;
  ...
  IP4_COPY_ADDRESS (&Dns4CfgData.StationIp, &NvData->NvmeofSubsysHostIP);
  IP4_COPY_ADDRESS (&Dns4CfgData.SubnetMask, &NvData->NvmeofSubsysHostSubnetMask);
  ...
  HostName = (CHAR16 *)AllocateZeroPool (NVMEOF_NAME_MAX_SIZE);
  if (HostName == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  DEBUG ((DEBUG_INFO, "Server Name: %s\n", HostName));
  ...
  //
  while (!IsDone && EFI_ERROR (gBS->CheckEvent (&Token.Event))) {
    Dns4->Poll (Dns4);
  }
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    IP4_COPY_ADDRESS (&NvData->NvmeofSubSystemIp.v4, Token.RspData.H2AData->IpList);
    DEBUG ((
      DEBUG_INFO,
      "Resolved IP Address: %d.%d.%d.%d\n",
      NvData->NvmeofSubSystemIp.v4.Addr[0],
      NvData->NvmeofSubSystemIp.v4.Addr[1],
      NvData->NvmeofSubSystemIp.v4.Addr[2],
      NvData->NvmeofSubSystemIp.v4.Addr[3]
      ));
    Status = EFI_SUCCESS;
  }
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    IP6_COPY_ADDRESS (&NvData->NvmeofSubSystemIp.v6, Token.RspData.H2AData->IpList);
    Status = EFI_SUCCESS;
  }
