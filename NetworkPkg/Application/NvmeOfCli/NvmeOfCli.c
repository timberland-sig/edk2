...
EFI_STATUS
EFIAPI
NvmeOfCompareReadWriteSyncData (
  UINTN  Argc
  )
{
...
EFI_STATUS
EFIAPI
NvmeOfCompareReadWriteAsyncData (
  UINTN  Argc
  )
{
  EFI_STATUS                Status = EFI_SUCCESS;
  ...
  )
{
  ...
  if (Flag) {
    ...
    DEBUG ((DEBUG_ERROR, "NvmeOfCliGetNsid: Error in NvmeOfCliGetNsid\n"));
  ...
  )
{
  ...
  if (Flag) {
    ...
    DEBUG ((DEBUG_ERROR, "NvmeOfCliGetQpair: Error in NvmeOfCliGetQpair\n"));
    Status = EFI_INVALID_PARAMETER;
    return Status;
  }
}
  ...
  )
{
  ...
  if (Flag) {
    ...
    DEBUG ((DEBUG_ERROR, "NvmeOfCliGetController: Error in NvmeOfCliGetController\n"));
    Status = EFI_INVALID_PARAMETER;
    return Status;
  }
}
  ...
  )
{
  ...
  if (Flag) {
    ...
    DEBUG ((DEBUG_ERROR, "NvmeOfCliGetControllerQpair: Error in NvmeOfCliGetControllerQpair\n"));
    Status = EFI_INVALID_PARAMETER;
    return Status;
  }
}
  ...
  )
{
  ...
  CHAR8                         Line1[1024];
  UINT32                        Attributes;
  NVMEOF_ATTEMPT_CONFIG_NVDATA  *Nvmeof_Attemptconfig;
  NVMEOF_GLOBAL_DATA            NvmeofData[1];

  Nvmeof_Attemptconfig = AllocateZeroPool (sizeof (NVMEOF_ATTEMPT_CONFIG_NVDATA) * MAX_ATTEMPTS);
  if (Nvmeof_Attemptconfig == NULL) {
  ...
  if (Flag == TRUE) {
    ...
    Status = gRT->SetVariable (
                    ...
                    &gNvmeOfConfigGuid,
                    Attributes,
                    sizeof (NVMEOF_ATTEMPT_CONFIG_NVDATA) * MAX_ATTEMPTS,
  ...
  )
{
  ...
  if (IsInvoke) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "ShellAppMain: CliLocateProtocol error\n"));
      DEBUG ((DEBUG_ERROR, "ShellAppMain: Failed to LOCATE protocol Status is %d .\n", Status));
      return Status;
    }
  }
