  ...
  )
{
  ...
  return spdk_nvme_ctrlr_cmd_admin_raw (
           ctrlr,
...
static int
nvme_fabric_prop_set_cmd_async (
  struct spdk_nvme_ctrlr  *ctrlr,
  uint32_t                offset,
  uint8_t                 size,
  uint64_t                value,
...
static int
nvme_fabric_prop_get_cmd_async (
  struct spdk_nvme_ctrlr  *ctrlr,
  uint32_t                offset,
  uint8_t                 size,
  spdk_nvme_reg_cb        cb_fn,
...
{
  return nvme_fabric_prop_set_cmd_async (
           ctrlr,
           offset,
...
int
nvme_fabric_ctrlr_set_reg_8_async (
  struct spdk_nvme_ctrlr  *ctrlr,
  uint32_t                offset,
  uint64_t                value,
  spdk_nvme_reg_cb        cb_fn,
...
int
nvme_fabric_ctrlr_get_reg_8_async (
  struct spdk_nvme_ctrlr  *ctrlr,
  uint32_t                offset,
  spdk_nvme_reg_cb        cb_fn,
  void                    *cb_arg
...
int
nvme_fabric_qpair_connect_async (
  ...
  )
{
  ...
  SPDK_STATIC_ASSERT (
    sizeof (nvmf_data->hostid) == sizeof (ctrlr->opts.extended_host_id),
  ...
  NVME_INIT_REQUEST (
    ...
    nvme_completion_poll_cb,
    status,
    NVME_PAYLOAD_CONTIG (nvmf_data, NULL),
    sizeof (*nvmf_data),
    0
    );
...
int
nvme_fabric_qpair_connect_poll (
  struct spdk_nvme_qpair  *qpair
  )
{
  struct nvme_completion_poll_status   *status;
  ...
  if (status->timed_out || spdk_nvme_cpl_is_error (&status->cpl)) {
    SPDK_ERRLOG (
      "Connect command failed, rc %d, trtype:%s adrfam:%s "
    ...
    if (status->timed_out) {
      ...
      SPDK_ERRLOG (
        "Connect command completed with error: sct %d, sc %d\n",
