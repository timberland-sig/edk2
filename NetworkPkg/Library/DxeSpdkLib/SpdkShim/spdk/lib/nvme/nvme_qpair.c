...
static const char *
nvme_get_string (
  const struct nvme_string  *strings,
  uint16_t                  value
  )
{
  ...
  )
{
  ...
  c = snprintf (
        ...
        size,
        "SGL %s %s 0x%" PRIx64,
        nvme_get_string (sgl_type, sgl->generic.type),
        nvme_get_string (sgl_subtype, sgl->generic.subtype),
        sgl->address
        );
  ...
  )
{
  ...
  switch ((int)cmd->opc) {
    ...
    case SPDK_NVME_OPC_GET_FEATURES:
      SPDK_NOTICELOG (
        "%s %s cid:%d cdw10:%08x %s\n",
        nvme_get_string (admin_opcode, cmd->opc),
        nvme_get_string (
          feat_opcode,
          cmd->cdw10_bits.set_features.fid
          ),
        cmd->cid,
        cmd->cdw10,
        dptr
    ...
    case SPDK_NVME_OPC_FABRIC:
      SPDK_NOTICELOG (
        "%s %s qid:%d cid:%d %s\n",
        nvme_get_string (admin_opcode, cmd->opc),
        nvme_get_string (fabric_opcode, fcmd->fctype),
        qid,
        fcmd->cid,
        dptr
        );
    ...
    default:
      SPDK_NOTICELOG (
        "%s (%02x) qid:%d cid:%d nsid:%x cdw10:%08x cdw11:%08x %s\n",
        nvme_get_string (admin_opcode, cmd->opc),
        cmd->opc,
        qid,
        cmd->cid,
  ...
  switch ((int)cmd->opc) {
    ...
    case SPDK_NVME_OPC_COMPARE:
      SPDK_NOTICELOG (
        ...
        "lba:%llu len:%d %s\n",
        nvme_get_string (io_opcode, cmd->opc),
        qid,
        cmd->cid,
        cmd->nsid,
...
spdk_nvme_cpl_get_status_string (
  const struct spdk_nvme_status  *status
  )
{
  const struct nvme_string  *entry;
  ...
  )
{
  ...
  SPDK_NOTICELOG (
    "%s (%02x/%02x) qid:%d cid:%d cdw0:%x sqhd:%04x p:%x m:%x dnr:%x\n",
 ...
 */
static void
_nvme_qpair_complete_abort_queued_reqs (
  struct spdk_nvme_qpair  *qpair
  )
{
  struct nvme_request  *req;
  ...
  )
{
  ...
  if (ret < 0) {
    if ((ret == -ENXIO) && (nvme_qpair_get_state (qpair) == NVME_QPAIR_DISCONNECTING)) {
      ...
      SPDK_ERRLOG (
        "CQ transport error %d (%s) on qpair id %hu\n",
        ret,
        spdk_strerror (-ret),
        qpair->id
        );
  ...
  )
{
  ...
  qpair->req_buf = spdk_zmalloc (
                     req_size_padded * num_requests,
                     64,
                     NULL,
