...
spdk_nvme_transport_register (
  const struct spdk_nvme_transport_ops  *ops
  )
{
  ...
  )
{
  ...
  if (rc != 0) {
    SPDK_ERRLOG (
      "transport %s returned non-zero for ctrlr_delete_io_qpair op\n",
      transport->ops.name
      );
    assert (false);
  }
...
nvme_transport_poll_group_create (
  const struct spdk_nvme_transport  *transport
  )
{
...
struct spdk_nvme_transport_poll_group *
nvme_transport_qpair_get_optimal_poll_group (
  const struct spdk_nvme_transport  *transport,
  struct spdk_nvme_qpair            *qpair
  )
{
...
int
nvme_transport_poll_group_remove (
  struct spdk_nvme_transport_poll_group  *tgroup,
  struct spdk_nvme_qpair                 *qpair
  )
{
...
{
  return tgroup->transport->ops.poll_group_process_completions (
                                  tgroup,
...
int
nvme_transport_poll_group_destroy (
  struct spdk_nvme_transport_poll_group  *tgroup
  )
{
...
int
nvme_transport_poll_group_get_stats (
  struct spdk_nvme_transport_poll_group       *tgroup,
...
void
nvme_transport_poll_group_free_stats (
  struct spdk_nvme_transport_poll_group       *tgroup,
  struct spdk_nvme_transport_poll_group_stat  *stats
  )
{
