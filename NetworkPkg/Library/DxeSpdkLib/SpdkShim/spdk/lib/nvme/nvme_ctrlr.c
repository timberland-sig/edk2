...
static int
nvme_ctrlr_identify_ns_iocs_specific_async (
  struct spdk_nvme_ns  *ns
  );

static int
nvme_ctrlr_identify_id_desc_async (
  struct spdk_nvme_ns  *ns
  );
...
static int
nvme_ns_cmp (
  struct spdk_nvme_ns  *ns1,
  struct spdk_nvme_ns  *ns2
  )
{
  if (ns1->id < ns2->id) {
    return -1;
...
{
  return nvme_transport_ctrlr_get_reg_4 (
           ctrlr,
           offsetof (struct spdk_nvme_registers, cc.raw),
           &cc->raw
           );
}
  ...
  )
{
  return nvme_transport_ctrlr_get_reg_4 (
           ctrlr,
           offsetof (struct spdk_nvme_registers, csts.raw),
           &csts->raw
           );
}
...
{
  return nvme_transport_ctrlr_get_reg_8 (
           ctrlr,
           offsetof (struct spdk_nvme_registers, cap.raw),
           &cap->raw
           );
}
  ...
  )
{
  return nvme_transport_ctrlr_get_reg_4 (
           ctrlr,
           offsetof (struct spdk_nvme_registers, vs.raw),
           &vs->raw
           );
}

int
nvme_ctrlr_get_cmbsz (
  struct spdk_nvme_ctrlr          *ctrlr,
  ...
  )
{
  return nvme_transport_ctrlr_get_reg_4 (
           ctrlr,
           offsetof (struct spdk_nvme_registers, cmbsz.raw),
           &cmbsz->raw
           );
}

int
nvme_ctrlr_get_pmrcap (
  struct spdk_nvme_ctrlr           *ctrlr,
  ...
  )
{
  return nvme_transport_ctrlr_get_reg_4 (
           ctrlr,
           offsetof (struct spdk_nvme_registers, bpinfo.raw),
           &bpinfo->raw
           );
}

int
nvme_ctrlr_set_bprsel (
  struct spdk_nvme_ctrlr           *ctrlr,
  union spdk_nvme_bprsel_register  *bprsel
  )
{
  return nvme_transport_ctrlr_set_reg_4 (
           ctrlr,
           offsetof (struct spdk_nvme_registers, bprsel.raw),
           bprsel->raw
           );
}

int
nvme_ctrlr_set_bpmbl (
  struct spdk_nvme_ctrlr  *ctrlr,
  uint64_t                bpmbl_value
  )
{
  return nvme_transport_ctrlr_set_reg_8 (
           ctrlr,
           offsetof (struct spdk_nvme_registers, bpmbl),
           bpmbl_value
           );
}
  ...
  )
{
  return nvme_transport_ctrlr_set_reg_4 (
           ctrlr,
           offsetof (struct spdk_nvme_registers, nssr),
           nssr_value
           );
}
  ...
  if (nvme_driver_init () == 0) {
    if (FIELD_OK (hostnqn)) {
      ...
      snprintf (
        opts->hostnqn,
        sizeof (opts->hostnqn),
        "nqn.2014-08.org.nvmexpress:uuid:%s",
        host_id_str
        );
    }
 ...
 */
static void
nvme_ctrlr_proc_remove_io_qpair (
  struct spdk_nvme_qpair  *qpair
  )
{
  struct spdk_nvme_ctrlr_process  *active_proc;
  ...
  TAILQ_FOREACH_SAFE (
    active_qpair,
    &active_proc->allocated_io_qpairs,
    per_process_tailq,
    tmp_qpair
    ) {
  ...
  if (user_opts) {
    ...
    if (opts.sq.vaddr) {
      if (opts.sq.buffer_size < (opts.io_queue_size * sizeof (struct spdk_nvme_cmd))) {
        NVME_CTRLR_ERRLOG (
          ctrlr,
          "sq buffer size %" PRIx64 " is too small for sq size %zx\n",
          opts.sq.buffer_size,
          (opts.io_queue_size * sizeof (struct spdk_nvme_cmd))
          );
        return NULL;
      }
    }
    ...
    if (opts.cq.vaddr) {
      if (opts.cq.buffer_size < (opts.io_queue_size * sizeof (struct spdk_nvme_cpl))) {
        NVME_CTRLR_ERRLOG (
          ctrlr,
          "cq buffer size %" PRIx64 " is too small for cq size %zx\n",
          opts.cq.buffer_size,
          (opts.io_queue_size * sizeof (struct spdk_nvme_cpl))
          );
        return NULL;
      }
    }
...
static int
nvme_ctrlr_set_intel_support_log_pages (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  int                         rc = 0;
  ...
  )
{
  ...
  if (ana_log_page_size > ctrlr->ana_log_page_size) {
    ...
    if (!new_buffer) {
      NVME_CTRLR_ERRLOG (
        ctrlr,
        "could not allocate ANA log page buffer, size %u\n",
        ana_log_page_size
        );
      return -ENXIO;
    }
    ...
    if (!new_buffer) {
      NVME_CTRLR_ERRLOG (
        ctrlr,
        "could not allocate a buffer to parse ANA descriptor, size %u\n",
        ana_log_page_size
        );
      return -ENOMEM;
    }
...
static int
nvme_ctrlr_update_ana_log_page (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
...
static int
nvme_ctrlr_update_ns_ana_states (
...
int
nvme_ctrlr_parse_ana_log_page (
  struct spdk_nvme_ctrlr           *ctrlr,
  spdk_nvme_parse_ana_log_page_cb  cb_fn,
  void                             *cb_arg
  )
{
...
static void
nvme_ctrlr_set_intel_supported_features (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
...
static void
nvme_ctrlr_set_arbitration_feature (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  uint32_t                            cdw11;
  ...
  if (spdk_nvme_ctrlr_cmd_set_feature (
        ctrlr,
...
static void
nvme_ctrlr_shutdown_get_cc_done (
  void                        *_ctx,
  uint64_t                    value,
  const struct spdk_nvme_cpl  *cpl
  )
...
static void
nvme_ctrlr_shutdown_get_csts_done (
  void                        *_ctx,
  uint64_t                    value,
  const struct spdk_nvme_cpl  *cpl
  )
  ...
  )
{
  ...
  NVME_CTRLR_ERRLOG (
    ctrlr,
    "did not shutdown within %u milliseconds\n",
    ctx->shutdown_timeout_ms
    );
  ...
  )
{
  ...
  if (!(ctrlr->cap.bits.css & (1u << ctrlr->opts.command_set))) {
    NVME_CTRLR_DEBUGLOG (
      ctrlr,
      "Requested I/O command set %u but supported mask is 0x%x\n",
      ctrlr->opts.command_set,
      ctrlr->cap.bits.css
      );
  ...
  )
{
  ...
  if (timeout_in_ms == NVME_TIMEOUT_KEEP_EXISTING) {
    if (!quiet) {
      NVME_CTRLR_DEBUGLOG (
        ctrlr,
        "setting state to %s (keeping existing timeout)\n",
  ...
  if (!quiet) {
    NVME_CTRLR_DEBUGLOG (
      ctrlr,
      "setting state to %s (timeout %" PRIu64 " ms)\n",
      nvme_ctrlr_state_string (ctrlr->state),
      timeout_in_ms
      );
  }
...
static void
nvme_ctrlr_free_iocs_specific_data (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
...
static void
nvme_ctrlr_set_doorbell_buffer_config_done (
  void                        *arg,
  const struct spdk_nvme_cpl  *cpl
  )
{
...
static int
nvme_ctrlr_set_doorbell_buffer_config (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  int       rc = 0;
  ...
  rc = nvme_ctrlr_cmd_doorbell_buffer_config (
         ctrlr,
         prp1,
         prp2,
         nvme_ctrlr_set_doorbell_buffer_config_done,
...
void
nvme_ctrlr_abort_queued_aborts (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
...
static int
nvme_ctrlr_get_zns_cmd_and_effects_log (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  int  rc;
...
static void
nvme_ctrlr_identify_zns_specific_done (
  void                        *arg,
  const struct spdk_nvme_cpl  *cpl
  )
{
 ...
 */
static int
nvme_ctrlr_identify_iocs_specific (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  int  rc;
  ...
  ctrlr->cdata_zns = spdk_zmalloc (
                       sizeof (*ctrlr->cdata_zns),
                       64,
                       NULL,
                       SPDK_ENV_SOCKET_ID_ANY,
                       SPDK_MALLOC_SHARE | SPDK_MALLOC_DMA
...
static int
nvme_ctrlr_construct_namespace (
  struct spdk_nvme_ctrlr  *ctrlr,
  uint32_t                nsid
  )
{
...
static void
nvme_ctrlr_identify_active_ns_swap (
  struct spdk_nvme_ctrlr  *ctrlr,
  uint32_t                *new_ns_list,
  size_t                  max_entries
  )
  ...
  )
{
  ...
  if ((ctrlr->vs.raw < SPDK_NVME_VERSION (1, 1, 0)) || ctrlr->quirks & NVME_QUIRK_IDENTIFY_CNS) {
     ...
     */
    ctx->page_count = spdk_divide_round_up (
                        ctrlr->cdata.nn + 1,
  ...
  rc         = nvme_ctrlr_cmd_identify (
                 ...
                 ctx->next_nsid,
                 0,
                 &ctx->new_ns_list[1024 * (ctx->page_count - 1)],
                 sizeof (struct spdk_nvme_ns_list),
                 nvme_ctrlr_identify_active_ns_async_done,
                 ctx
                 );
  ...
  )
{
  ...
  return nvme_ctrlr_cmd_identify (
           ns->ctrlr,
...
static void
nvme_ctrlr_identify_ns_zns_specific_async_done (
  void                        *arg,
  const struct spdk_nvme_cpl  *cpl
  )
{
...
static int
nvme_ctrlr_identify_namespaces_iocs_specific (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  if (!nvme_ctrlr_multi_iocs_enabled (ctrlr)) {
...
static int
nvme_ctrlr_identify_id_desc_namespaces (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  uint32_t             nsid;
  ...
  )
{
  if (spdk_nvme_ctrlr_is_fabrics (ctrlr)) {
    if (ctrlr->cdata.nvmf_specific.ioccsz < 4) {
      NVME_CTRLR_ERRLOG (
        ctrlr,
        "Incorrect IOCCSZ %u, the minimum value should be 4\n",
  ...
  )
{
  ...
  if (ctrlr->opts.num_io_queues > SPDK_NVME_MAX_IO_QUEUES) {
    NVME_CTRLR_NOTICELOG (
      ctrlr,
      "Limiting requested num_io_queues %u to max %d\n",
  ...
  rc = nvme_ctrlr_cmd_set_num_queues (
         ctrlr,
         ctrlr->opts.num_io_queues,
         nvme_ctrlr_set_num_queues_done,
         ctrlr
...
static void
nvme_ctrlr_set_keep_alive_timeout_done (
  void                        *arg,
  const struct spdk_nvme_cpl  *cpl
  )
{
  ...
  if (spdk_nvme_cpl_is_error (cpl)) {
        ...
        (cpl->status.sc == SPDK_NVME_SC_INVALID_FIELD))
    {
      ...
      NVME_CTRLR_ERRLOG (
        ctrlr,
        "Keep alive timeout Get Feature failed: SC %x SCT %x\n",
        cpl->status.sc,
        cpl->status.sct
        );
...
static int
nvme_ctrlr_set_keep_alive_timeout (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  int  rc;
...
static int
nvme_ctrlr_clear_changed_ns_log (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  struct nvme_completion_poll_status  *status;
  ...
  if (!buffer) {
    NVME_CTRLR_ERRLOG (
      ctrlr,
      "Failed to allocate buffer for getting "
      "changed ns log.\n"
      );
    return rc;
  }
...
void
nvme_ctrlr_complete_queued_async_events (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  ...
  )
{
  ...
  rc = nvme_ctrlr_cmd_set_async_event_config (
         ctrlr,
         config,
         nvme_ctrlr_configure_aer_done,
         ctrlr
  ...
  )
{
  ...
  ctrlr_proc = spdk_zmalloc (
                 sizeof (struct spdk_nvme_ctrlr_process),
                 64,
                 NULL,
                 SPDK_ENV_SOCKET_ID_ANY,
                 SPDK_MALLOC_SHARE
...
static void
nvme_ctrlr_process_init_vs_done (
  void                        *ctx,
  uint64_t                    value,
  const struct spdk_nvme_cpl  *cpl
  )
...
static void
nvme_ctrlr_process_init_cap_done (
  void                        *ctx,
  uint64_t                    value,
  const struct spdk_nvme_cpl  *cpl
  )
...
static void
nvme_ctrlr_process_init_check_en (
  void                        *ctx,
  uint64_t                    value,
  const struct spdk_nvme_cpl  *cpl
  )
...
static void
nvme_ctrlr_process_init_set_en_0 (
  void                        *ctx,
  uint64_t                    value,
  const struct spdk_nvme_cpl  *cpl
  )
...
static void
nvme_ctrlr_process_init_set_en_0_read_cc (
  void                        *ctx,
  uint64_t                    value,
  const struct spdk_nvme_cpl  *cpl
  )
...
static void
nvme_ctrlr_process_init_wait_for_ready_1 (
  void                        *ctx,
  uint64_t                    value,
  const struct spdk_nvme_cpl  *cpl
  )
...
static void
nvme_ctrlr_process_init_wait_for_ready_0 (
  void                        *ctx,
  uint64_t                    value,
  const struct spdk_nvme_cpl  *cpl
  )
  ...
  )
{
  ...
  if (rc) {
    NVME_CTRLR_ERRLOG (
      ctrlr,
      "Ctrlr operation failed with error: %d, ctrlr state: %d (%s)\n",
      rc,
      ctrlr->state,
      nvme_ctrlr_state_string (ctrlr->state)
      );
      ...
      (ticks > ctrlr->state_timeout_tsc))
  {
    NVME_CTRLR_ERRLOG (
      ctrlr,
      "Initialization timed out in state %d (%s)\n",
      ctrlr->state,
      nvme_ctrlr_state_string (ctrlr->state)
      );
    return -1;
...
int
nvme_robust_mutex_init_recursive_shared (
  pthread_mutex_t  *mtx
  )
{
  ...
  )
{
      ...
      ((ctrlr->opts.admin_queue_size % SPDK_NVME_ADMIN_QUEUE_QUIRK_ENTRIES_MULTIPLE) != 0))
  {
    ...
    ctrlr->opts.admin_queue_size = SPDK_ALIGN_CEIL (
                                     ctrlr->opts.admin_queue_size,
  ...
  )
{
  ...
  res = spdk_nvme_ctrlr_cmd_security_receive (
          ctrlr,
          secp,
          spsp,
          nssf,
  ...
  )
{
  ...
  res = spdk_nvme_ctrlr_cmd_security_send (
          ctrlr,
          secp,
          spsp,
          nssf,
