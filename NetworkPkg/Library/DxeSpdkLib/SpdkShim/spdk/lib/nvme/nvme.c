...
static void
nvme_ctrlr_detach_async_finish (
  struct spdk_nvme_ctrlr  *ctrlr
  )
{
  ...
  )
{
   ...
   * qpair to NULL.
   */
  active_proc->timeout_cb_fn (
                 active_proc->timeout_cb_arg,
                 ctrlr,
  ...
  )
{
  ...
  if (spdk_process_is_primary ()) {
    ...
    if (g_spdk_nvme_driver != NULL) {
      ...
      g_spdk_nvme_driver = spdk_memzone_reserve (
                             SPDK_NVME_DRIVER_NAME,
                             sizeof (struct nvme_driver),
                             socket_id,
  ...
  )
{
  ...
  *str += strspn (*str, whitespace);
  ...
  if (key_len >= key_buf_size) {
    SPDK_ERRLOG (
      "Key length %zu greater than maximum allowed %zu\n",
      key_len,
      key_buf_size - 1
      );
    return 0;
  ...
  *str   += key_len + 1;     /* Skip key: */
  ...
  if (val_len >= val_buf_size) {
    SPDK_ERRLOG (
      "Value length %zu greater than maximum allowed %zu\n",
      val_len,
      val_buf_size - 1
      );
    return 0;
  ...
  while (*str != '\0') {
    ...
    if (strcasecmp (key, "trtype") == 0) {
      ...
      if (val_len > SPDK_NVMF_TRADDR_MAX_LEN) {
        SPDK_ERRLOG (
          "traddr length %zu greater than maximum allowed %u\n",
          val_len,
      ...
      if (val_len > SPDK_NVMF_TRSVCID_MAX_LEN) {
        SPDK_ERRLOG (
          "trsvcid length %zu greater than maximum allowed %u\n",
          val_len,
      ...
      if (val_len > SPDK_NVMF_NQN_MAX_LEN) {
        SPDK_ERRLOG (
          "subnqn length %zu greater than maximum allowed %u\n",
          val_len,
  ...
  )
{
  ...
  while (*str != '\0') {
    ...
    if (strcasecmp (key, "trtype") == 0) {
      ...
      if (val_len > SPDK_NVMF_TRADDR_MAX_LEN) {
        SPDK_ERRLOG (
          "hostaddr length %zu greater than maximum allowed %u\n",
          val_len,
          SPDK_NVMF_TRADDR_MAX_LEN
          );
        return -EINVAL;
