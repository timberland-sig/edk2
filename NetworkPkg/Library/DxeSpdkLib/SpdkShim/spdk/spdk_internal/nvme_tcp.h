...
typedef void (*nvme_tcp_qpair_xfer_complete_cb)(
  void  *cb_arg
  );
...
SPDK_STATIC_ASSERT (
  offsetof (
    struct nvme_tcp_pdu,
    sock_req
    ) + sizeof (struct spdk_sock_request) == offsetof (struct nvme_tcp_pdu, iov),
...
static uint32_t
nvme_tcp_pdu_calc_header_digest (
...
static inline void
_nvme_tcp_sgl_get_buf (
  struct spdk_iov_sgl  *s,
  void                 **_buf,
  uint32_t             *_buf_len
  )
{
...
static inline bool
_nvme_tcp_sgl_append_multi_with_md (
  struct spdk_iov_sgl        *s,
  struct iovec               *iov,
  int                        iovcnt,
  uint32_t                   data_len,
  ...
  )
{
  ...
  if (s->iov_offset >= data_len) {
    ...
    rc = spdk_dif_set_md_interleave_iovs (
           s->iov,
           s->iovcnt,
           iov,
           iovcnt,
           s->iov_offset,
           data_len - s->iov_offset,
           &mapped_len,
           dif_ctx
           );
    if (rc < 0) {
  ...
  )
{
  ...
  if (spdk_likely (!pdu->dif_ctx)) {
    ...
    if (!_nvme_tcp_sgl_append_multi_with_md (
           sgl,
           pdu->data_iov,
           pdu->data_iovcnt,
           pdu->data_len,
  ...
  )
{
  ...
  if (ret < 0) {
    ...
    if (errno != ECONNRESET) {
      SPDK_ERRLOG (
        "spdk_sock_recv() failed, errno %d: %s\n",
        errno,
        spdk_strerror (errno)
        );
    }
  ...
  )
{
  ...
  if (ret < 0) {
    ...
    if (errno != ECONNRESET) {
      SPDK_ERRLOG (
        "spdk_sock_readv() failed, errno %d: %s\n",
        errno,
        spdk_strerror (errno)
        );
    }
  ...
  )
{
  ...
  iovcnt = nvme_tcp_build_payload_iovs (
             iov,
             NVME_TCP_MAX_SGL_DESCRIPTORS + 1,
             pdu,
             pdu->ddgst_enable,
             NULL
             );
